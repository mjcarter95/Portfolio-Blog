...
In the interactive plot below, 80 samples are drawn from a Gaussian distribution with true mean μ = 10 and variance σ² = 1. The slider controls our hypothesis for the mean. As you move the slider away from the true mean, the red curve shifts, and the negative log-likelihood (NLL) value updates in real time. Notice how the NLL increases the further our hypothesis moves from μ = 10, indicating a progressively poorer fit to the observed data.
