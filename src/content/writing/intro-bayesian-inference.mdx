...
def gamma_likelihood(k, theta, data):
    ...
    """
    return np.prod(gamma.pdf(data, a=k, scale=theta))
```
...
Below we draw synthetic wait times from a known Gamma distribution and fit the parameters $k$ and $\theta$ using Bayesian inference implemented in the probabilistic programming language Stan. Stan allows us to specify the model in a concise mathematical form and then automatically perform posterior sampling using state-of-the-art Markov Chain Monte Carlo (MCMC) algorithms.

We visualise:
- a **histogram** of the data (as a density),
- the **fitted Gamma pdf** overlay,
- a vertical line at the **fitted mean** $\hat{\mu} = \hat{k} \hat{\theta}$.
